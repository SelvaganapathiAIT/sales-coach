...
DROP POLICY IF EXISTS "Users can view their own coaching notes" ON public.coaching_notes;
CREATE POLICY "Users can view their own coaching notes"
ON public.coaching_notes
FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can create their own coaching notes" ON public.coaching_notes;
CREATE POLICY "Users can create their own coaching notes"
ON public.coaching_notes
FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own coaching notes" ON public.coaching_notes;
CREATE POLICY "Users can update their own coaching notes"
ON public.coaching_notes
FOR UPDATE USING (auth.uid() = user_id);

-- RLS policies for user_coaching_context
DROP POLICY IF EXISTS "Users can view their own coaching context" ON public.user_coaching_context;
CREATE POLICY "Users can view their own coaching context"
ON public.user_coaching_context
FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can create their own coaching context" ON public.user_coaching_context;
CREATE POLICY "Users can create their own coaching context"
ON public.user_coaching_context
FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own coaching context" ON public.user_coaching_context;
...
CREATE INDEX IF NOT EXISTS idx_coaching_notes_user_id ON public.coaching_notes(user_id);
CREATE INDEX IF NOT EXISTS idx_coaching_notes_session_id ON public.coaching_notes(session_id);
CREATE INDEX IF NOT EXISTS idx_coaching_notes_note_type ON public.coaching_notes(note_type);
...
DROP FUNCTION IF EXISTS public.get_user_coaching_context(UUID);
